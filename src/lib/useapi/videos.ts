...
export type GenerateVideoInput = { prompt: string; model: VideoModel; aspectRatio: "portrait" | "landscape" | "1:1" | "4:3" | "3:4"; duration: 4 | 6 | 8 | 10; startImage: string; characterRef?: string; voiceRef?: string; replyUrl: string; replyRef: string };

export async function generateVideo(input: GenerateVideoInput) {
  const response = await useApiRequest<unknown>("/videos", { method: "POST", retrySafe: false, body: {
    prompt: input.prompt,
    model: input.model,
